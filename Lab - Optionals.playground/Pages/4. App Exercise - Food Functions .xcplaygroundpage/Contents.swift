...
func meal(for mealName: String) -> Meal? {
    if let meal = meals[mealName]{
        return meal
    } else {
        return nil
    }
 ...
 */
func getMealFromUserDefaults() -> [String: Any]? {
    if let dictionary = UserDefaults.standard.dictionary(forKey: "mealLog") {
        return dictionary
    } else {
        return nil
    }
